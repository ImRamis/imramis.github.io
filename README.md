...
- **"Choose your lens" gateway** — pick a discipline; accent colour, hero copy,
  rotating roles (red / blue / purple), focus areas, toolkit, default project
  filter and an animated terminal all adapt. Switch lenses anytime from the nav.
...
- **Hybrid, but cheap** — static frontend on free GitHub Pages + a Git-based
...
For live editing on ramis.me, set up a GitHub OAuth app + a small OAuth proxy
...
- **Blog posts**: `content/posts/*.md` (via `/admin`) → `build-posts.mjs`
